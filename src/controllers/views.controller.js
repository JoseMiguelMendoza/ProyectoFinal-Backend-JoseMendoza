...
export const renderUserManageForAdmin = async(req, res) => {
    const user = await UserService.findUserById(req.session.passport.user)
    const allUsers = await UserService.getUsers()
    const notAdminsInMyLawn = []
    for(const user of allUsers){
        if(user.role === 'Usuario/a'){
            notAdminsInMyLawn.push(user)
        }
    }

    const isThereUsers = notAdminsInMyLawn.length > 0
    ...
    res.render('userManage', {
        ...
        role: user.role,
        checkingRole: user.role === 'Administrador/a' ? true : false,
        allUsers: notAdminsInMyLawn,
        isThereUsers: isThereUsers
    })
}